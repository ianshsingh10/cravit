...
import { Plus, Minus, X, Loader2, Star } from "lucide-react"; // Import Loader2 icon
  ...
  const handleConfirmAddToCart = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        alert(
          `${quantity} x ${item.itemName} (${orderType}) added successfully!`
        );
        handleCloseModal();
      }
    } catch (error) {
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-4 bg-gray-100 dark:bg-gray-700 rounded-full p-1">
                  <button
                    ...
                    className="p-2 rounded-full disabled:opacity-50 disabled:cursor-not-allowed hover:bg-gray-200 dark:hover:bg-gray-600 transition"
                    disabled={quantity === 1}
                  >
                    <Minus size={16} />
                  </button>
