...
export default function RegisterPage() {
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 flex flex-col items-center p-4 pt-15">
      <div className="max-w-md w-full mx-auto bg-white dark:bg-gray-800 p-8 rounded-2xl shadow-lg">
        <div className="text-center mb-8">
          ...
          <p className="text-gray-500 dark:text-gray-400">Join the craVIT community today!</p>
        </div>
        ...
        {step === "form" ? (
          <form onSubmit={handleSendOtp} className="space-y-4">
            <div className="relative">
              ...
              <input type="text" placeholder="Full Name" value={name} onChange={(e) => setName(e.target.value)} className="text-black dark:text-white bg-white dark:bg-gray-700 pl-10 border border-gray-300 dark:border-gray-600 p-3 rounded-lg w-full focus:ring-2 focus:ring-orange-500 focus:border-transparent transition" required />
            </div>
            <div className="relative">
              <Mail className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" size={20} />
              <input type="email" placeholder="Email Address" value={email} onChange={(e) => setEmail(e.target.value)} className="text-black dark:text-white bg-white dark:bg-gray-700 pl-10 border border-gray-300 dark:border-gray-600 p-3 rounded-lg w-full focus:ring-2 focus:ring-orange-500 focus:border-transparent transition" required />
            </div>
            <div className="relative">
          ...
          <form onSubmit={handleVerifyOtp} className="space-y-4">
            ...
            <button type="submit" disabled={isLoading} className="w-full flex justify-center items-center gap-2 bg-green-500 text-white font-bold px-4 py-3 rounded-lg hover:bg-green-600 transition-colors duration-300 disabled:bg-green-400">
              {isLoading ? <><Loader2 className="animate-spin" size={20}/> Verifying...</> : "Verify & Register"}
            </button>
