...
import { X, Loader2, CheckCircle, Clock, XCircle, RefreshCw } from "lucide-react";
import { StatusIndicator } from "./StatusIndicator";

export const OrderDetailsModal = ({ order, onClose, onCancelOrder, isActionLoading }) => {
    ...
    const OrderTracker = ({ statusHistory }) => {
        const getStatusTimelineIcon = (status) => {
            const config = {
                Delivered: { icon: <CheckCircle className="w-4 h-4 text-green-500" />, bg: 'bg-green-500/20' },
                Preparing: { icon: <Clock className="w-4 h-4 text-yellow-500" />, bg: 'bg-yellow-500/20' },
                Accepted: { icon: <Clock className="w-4 h-4 text-yellow-500" />, bg: 'bg-yellow-500/20' },
                'Out for Delivery': { icon: <Clock className="w-4 h-4 text-yellow-500" />, bg: 'bg-yellow-500/20' },
                Pending: { icon: <Clock className="w-4 h-4 text-blue-500" />, bg: 'bg-blue-500/20' },
                Cancelled: { icon: <XCircle className="w-4 h-4 text-red-500" />, bg: 'bg-red-500/20' },
                Refunded: { icon: <RefreshCw className="w-4 h-4 text-gray-500" />, bg: 'bg-gray-500/20' },
            };
            return config[status] || { icon: <Clock className="w-4 h-4 text-gray-500" />, bg: 'bg-gray-500/20' };
        };

        return (
            <div>
                <h3 className="text-md font-semibold text-gray-600 dark:text-gray-300 mb-3">Order Tracker</h3>
                        ...
                        return (
                            <div key={index} className="flex items-center gap-4">
                                <div className="flex flex-col items-center">
                                    <div className={`w-8 h-8 rounded-full flex items-center justify-center ${timelineIcon.bg}`}>
                                        {timelineIcon.icon}
                                    </div>
                                ...
                                <div>
                                    <p className="font-semibold text-gray-800 dark:text-gray-200">{historyItem.status}</p>
    ...
    return (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={onClose} className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex justify-center items-center p-4">
            <motion.div initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.9, y: 20, opacity: 0 }} onClick={(e) => e.stopPropagation()} className="relative bg-white dark:bg-gray-800 rounded-2xl shadow-xl w-full max-w-lg">
                ...
                <div className="p-6 space-y-6 max-h-[60vh] overflow-y-auto scrollbar-hide [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden">
